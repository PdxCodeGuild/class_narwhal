...
First download the file from [here](https://upload.wikimedia.org/wikipedia/en/7/7d/Lenna_%28test_image%29.png) 
and place it in the same directory as your code (or save it anywhere and use an absolute path when opening it). 
If you don't have pillow installed, run `pip install pillow` in a terminal. You can check if pillow using `pip show pillow`.

Use the formula for converting to greyscale and the code below. Remember that Pillow uses `ints` for RGB values, 
in the range of 0-255, whereas your math will often use `floats`. 
...
You can find more documentation [here](http://pillow.readthedocs.io/en/4.2.x/reference/ImageDraw.html).
